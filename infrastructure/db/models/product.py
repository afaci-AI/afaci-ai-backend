...
from sqlalchemy import Column, Double, ForeignKey, String, UniqueConstraint
...
class Product(Base):
    ...
    __table_args__ = (
        UniqueConstraint("name", "region_id", name="uq_product_name_region"),
    )
...
class Nutrient(Base):
    ...
    product_id = Column(
        "id_product", UUID(as_uuid=True), ForeignKey("products.id"), nullable=False
    )
    nutrient_name_id = Column(
        "id_name_component",
        UUID(as_uuid=True),
        ForeignKey("nutrients_names.id"),
        nullable=False,
    )
