...
async def list_reference_proteins(db: Annotated[AsyncSession, Depends(get_db)]):
    rows = (
        (
            await db.execute(
                text(
    ...
    for rp in rows:
        vals = (
            (
                await db.execute(
                    text(
                        "SELECT amino_acid, value, sort_order FROM reference_protein_values "
        ...
        result.append(
            {
                ...
                "values": [
                    {"amino_acid": v["amino_acid"], "value": v["value"]} for v in vals
                ],
            }
        )
    return result
...
async def list_recipes(db: Annotated[AsyncSession, Depends(get_db)]):
    recipes = (
        (
            await db.execute(
                text(
...
async def optimize_cost_endpoint(
    ...
    return await optimize_recipe_cost(
        db, req.reference_protein_id, candidates, constraints
    )
