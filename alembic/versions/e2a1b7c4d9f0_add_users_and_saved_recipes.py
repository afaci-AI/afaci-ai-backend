...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("group_id", UUID(as_uuid=True),
                  sa.ForeignKey("recipe_groups.id", ondelete="SET NULL"), nullable=True),
        sa.Column("name", sa.String(), nullable=False),
        sa.Column("reference_protein_id", UUID(as_uuid=True),
                  sa.ForeignKey("reference_proteins.id"), nullable=False),
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False,
                  server_default=sa.func.now()),
    ...
    op.create_table(
        "saved_recipe_items",
        sa.Column("id", UUID(as_uuid=True), primary_key=True),
        sa.Column("saved_recipe_id", UUID(as_uuid=True),
                  sa.ForeignKey("saved_recipes.id", ondelete="CASCADE"), nullable=False),
    ...
    op.create_index("ix_saved_recipe_items_recipe_id", "saved_recipe_items",
                    ["saved_recipe_id"])
